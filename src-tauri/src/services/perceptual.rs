...
pub fn bucket_by_prefix(rows: impl IntoIterator<Item = (i64, u64)>) -> Vec<(u16, Vec<(i64, u64)>)> {
    let mut buckets: std::collections::HashMap<u16, Vec<(i64, u64)>> =
...
mod tests {
    ...
    fn tiny_edits_yield_small_hamming() {
        ...
        assert!(
            hamming(dhash(&base), dhash(&shifted)) <= 4,
            "亮度偏移后差异应很小"
        );
    }
    ...
    fn resize_robustness() {
        ...
        for (x, _y, p) in big.enumerate_pixels_mut() {
            *p = Rgb(if x < 240 {
                [10, 20, 30]
            } else {
                [200, 210, 220]
    ...
    fn prefix_bucketing_groups_similar() {
        ...
        assert_eq!(hamming(pair.1[0].1, pair.1[1].1), 1, "同前缀组内两条应接近");
    }
}
