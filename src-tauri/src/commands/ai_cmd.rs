...
#[tauri::command]
pub async fn ai_list_models(base_url: String, api_key: String, api_mode: String) -> AppResult<Vec<String>> {
    tauri::async_runtime::spawn_blocking(move || ai_cloud::list_models(&base_url, &api_key, &api_mode))
        .await
...
#[tauri::command]
pub fn ai_create_batch(state: State<AppState>, asset_ids: Vec<i64>, mode: String) -> AppResult<AiBatch> {
    if asset_ids.is_empty() {
        return Err(AppError::msg("未选择任何素材"));
    }
    if mode != "cloud" && mode != "local" && mode != "manual" {
        return Err(AppError::msg("非法打标模式"));
    }
    let conn = lock_db(&state)?;
    // 批量上限（PRD 风险控制：防 API 成本失控）
    let limit = settings::get_settings(&conn)?.ai.batch_limit;
...
#[tauri::command]
pub async fn ai_start_batch(app: AppHandle, state: State<'_, AppState>, batch_id: i64, limit: Option<i64>) -> AppResult<AiBatch> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let all = {
            ...
            if batch.mode != "cloud" {
                return Err(AppError::msg("本地模型打标将于 M2 交付"));
            }
            // v2.12：仅执行中拒绝；done/cancelled 允许续跑剩余 pending 建议
            if batch.status == "processing" {
                return Err(AppError::msg("批次正在执行中"));
            }
            let has_pending = ai::list_suggestions(&conn, batch_id)?.iter().any(|s| s.status == "pending");
            if !has_pending {
                return Err(AppError::msg("没有待处理的建议（已全部确认或拒绝）"));
            }
...
#[tauri::command]
pub fn ai_confirm_suggestion(state: State<AppState>, id: i64, tags: CategorizedTags) -> AppResult<()> {
    let conn = lock_db(&state)?;
...
#[tauri::command]
pub fn ai_apply_tags(state: State<AppState>, asset_ids: Vec<i64>, tags: CategorizedTags) -> AppResult<()> {
    let conn = lock_db(&state)?;
