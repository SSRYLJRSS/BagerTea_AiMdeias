...
| 三目标 code-gate 与手动候选包 workflow | 验证基线 `1c33ba899b3db186835667907bd9bc7ee5ada7ec` 的 code-gate 与 Windows smoke 均已通过；候选包 workflow 尚未运行 | 同一 SHA 的 [code-gate run 36099151444](https://github.com/SSRYLJRSS/BagerTea_AiMdeias/actions/runs/36099151444) 中 frontend、Windows x64、macOS Apple Silicon、Linux x64 全绿；手动 Windows [smoke run 36101204016](https://github.com/SSRYLJRSS/BagerTea_AiMdeias/actions/runs/36101204016) 全绿。`smoke.yml` 的 push/PR 自动触发仍限于 `main`。GitHub `main` 无有效保护规则/Required checks，因此 CI 不是合并阻断门禁。候选包 workflow 虽存在于工作分支，但默认分支未注册该 workflow，当前不能从 Actions 手动运行；需先让 workflow 进入默认分支后再派发到候选 ref |
...
最新已完成的 source code gate 是验证基线 `1c33ba899b3db186835667907bd9bc7ee5ada7ec` 的 [run 36099151444](https://github.com/SSRYLJRSS/BagerTea_AiMdeias/actions/runs/36099151444)：frontend、Linux x64、macOS Apple Silicon、Windows x64 四个 jobs 全部通过；同一 SHA 的 Windows [smoke run 36101204016](https://github.com/SSRYLJRSS/BagerTea_AiMdeias/actions/runs/36101204016) 也通过。该提交相对代码提交 `0aa1039` 仅增加进度记录文档，没有改变应用源码。源码提交 `fd319ebcfc551b791664fa69eccfc1512faf7be3` 的 [run 36000987683](https://github.com/SSRYLJRSS/BagerTea_AiMdeias/actions/runs/36000987683) 也全部通过；该提交只调整 `SettingsPage.test.tsx` 的异步测试等待，不改产品行为，其本地门禁结果见 §2.4。Windows MSI/NSIS 已从 `1c33ba8` 本机生成用于构建验证（摘要见上表），但没有生成三端候选包 manifest，也未安装、启动或分发。远端 `main` 仍为 `683628ae25feb610fdd84aeb37b036e18187a5fe`，未创建 PR 或合并。
...
| P4 同一源码验证 | 验证基线 `1c33ba899b3db186835667907bd9bc7ee5ada7ec` 的 frontend、Windows x64、macOS Apple Silicon、Linux x64 code-gate 与同 SHA Windows smoke 全绿；同一 SHA 本机 MSI/NSIS 构建成功 | 代码级验证和本机 Windows 打包链路已完成；包尚未安装/启动验收。仍无三端候选 manifest、真机 UAT、分发或许可审查证据。CI 不是 `main` 的 Required check。 |
