...
describe("Thumbnail 高清缓存与代际保护", () => {
  it("命中 ready 缓存的重挂载：立即显示高清，hdReady=true（跳过重复淡入）", async () => {
    ...
    const first = render(<Thumbnail assetId={1} placeholderPath="d:/t/p1.jpg" alt="a" />);
    fireVisible();
    // 第一次请求完成 → 高清显示
    await waitFor(() => {
    ...
    const second = render(<Thumbnail assetId={1} placeholderPath="d:/t/p1.jpg" alt="a" />);
  ...
  it("代际保护：卸载后旧 Promise 完成不更新新素材（不抛错、不串图）", async () => {
    ...
    const first = render(<Thumbnail assetId={1} placeholderPath={null} alt="a" />);
    fireVisible(); // 发起请求（挂起）
    // 卸载：代际推进
    first.unmount();
    // 让旧请求完成——应被代际保护丢弃，不抛错
    ...
    await waitFor(() => {
      ...
      const second = render(<Thumbnail assetId={2} placeholderPath={null} alt="b" />);
  ...
  it("占位图加载失败触发高清请求", async () => {
    vi.mocked(getThumbnailUrl).mockResolvedValue("asset://hd/1");
    const { container } = render(<Thumbnail assetId={1} placeholderPath="d:/t/broken.jpg" alt="a" />);
  ...
  it("同一素材并发请求合并为一个（single-flight）", async () => {
    ...
    render(<Thumbnail assetId={7} placeholderPath={null} alt="a" />);
    render(<Thumbnail assetId={7} placeholderPath={null} alt="b" />);
    fireVisible();
    // 只发一次底层 IPC
    expect(vi.mocked(getThumbnailUrl)).toHaveBeenCalledTimes(1);
